...
// 毫秒延迟函数，需要您在ti_msp_dl_config.c中实现mspm0_delay_ms
void delay_ms(uint32_t ms)
{
    // 调用MSPM0的延迟函数，需要在ti_msp_dl_config.c中实现
    // 或者使用其他可用的延迟函数
    for(uint32_t i = 0; i < ms; i++)
    {
        // 简单的延迟循环，1ms大约需要32000次循环（假设32MHz时钟）
        for(uint32_t j = 0; j < 32000; j++)
        {
            __asm("nop");
        }
    }
