...
*@author:		Pan ZhiKang
*@date:			2022-10-26
*@note:			移植OLED模块请更改OLED.h中引脚相关配置宏
...
uint8_t OLED_GRAM[128][8];//每页包含128位,共8页
//[0]0 1 2 3 ... 127
//[1]0 1 2 3 ... 127
//[2]0 1 2 3 ... 127
//[3]0 1 2 3 ... 127
...
static void OLED_Pin_Config(void)
{
	...
	OLED_SCL_High();									//上电保护，防止使能芯片
	OLED_DC_High();
	OLED_CS_High();
}

/*********************************************************************
*@brief:		SPI通信格式向OLED读写命令/数据
*@param:		所要写入的数据/命令Data
*@param:		控制写入命令或数据标识符WR_Command，该参数只能为以下：
*				@arg:	Write_Data			表示向OLED写入命令
*				@arg:	Write_Command		表示向OLED写入数据
*@retval:		返回所读取的数据
*********************************************************************/
static void OLED_WriteReadByte(uint8_t Data,uint8_t WR_Command)
{
	...
	if(WR_Command)										//判断主机向OLED写入数据还是命令
		OLED_DC_High();
	else
		OLED_DC_Low();
...
void OLED_Refresh_GRAM(void)
{
	...
	for(Page_ctr=0;Page_ctr<8;Page_ctr++)
	{
		...
		OLED_WriteReadByte(0x00,Write_Command);      		//设置显示位置—列低地址
		OLED_WriteReadByte(0x10,Write_Command);      		//设置显示位置—列高地址
...
void OLED_AllScreen_Clear(void)
{
	...
	for(Page_ctr=0;Page_ctr<8;Page_ctr++)					//将8页128*8*8个字节数据全赋0x00
	{
		for(Pixel_ctr=0;Pixel_ctr<128;Pixel_ctr++)
		{
...
/*********************************************************************
*@brief:		OLED修改单一像素点
*@param:		X：		像素点X坐标,该数值的有效范围0~127
*@param:		Y：		像素点Y坐标,该数值的有效范围0~63
*@param:		OP:		选择是否填充该像素点,true为是,false为否
*@retval:		None
*********************************************************************/
...
/*********************************************************************
*@brief:		OLED修改区域像素
*@param:		X0：	区域起始X坐标,该数值的有效范围0~127
*@param:		Y0：	区域起始Y坐标,该数值的有效范围0~63
*@param:		X1：	区域末尾X坐标,该数值的有效范围0~127
*@param:		Y1：	区域末尾Y坐标,该数值的有效范围0~63
*@param:		OP:		选择是否填充该像素点,该参数值只能为以下：
*				@arg:	OLED_Clear
*				@arg:	OLED_Fill
*@retval:		None
*********************************************************************/
void OLED_EditArea(uint8_t X0,uint8_t Y0,uint8_t X1,uint8_t Y1,uint8_t OP)  
{
	...
	for(X=X0;X<=X1;X++)					//通过批量编辑像素点来修改选中区域像素
	{
		for(Y=Y0;Y<=Y1;Y++)
		{
...
/*********************************************************************
*@brief:		OLED画线函数
*@param:		x1:		区域起始X坐标,该数值的有效范围0~127
*@param:		y1:		区域起始Y坐标,该数值的有效范围0~63
*@param:		x2:		区域末尾X坐标,该数值的有效范围0~127
*@param:		y2:		区域末尾Y坐标,该数值的有效范围0~63
*@param:		OP:		是否镂空,true为镂空,false为填充
*@retval:		None
*********************************************************************/
...
/*********************************************************************
*@brief:		OLED画圆函数
*@param:		X0:		圆心坐标X,该数值的有效范围0~127
*@param:		Y0:		圆心坐标Y,该数值的有效范围0~63
*@param:		R:		圆半径
*@retval:		None
*********************************************************************/
...
*@param:		X:				字符X坐标,该数值的有效范围0~127
*@param:		Y:				字符行坐标,该数值的有效范围0~63
*@param:		Char:			欲显示的字符
*@param:		Fonty:			字体格式,该值可选的参数仅有以下
*@arg:							OLED_FONTSIZE_12X6
*@arg:							OLED_FONTSIZE_16X8
*@arg:							OLED_FONTSIZE_24X12
*@param:		OP:				false为正常显示，true为反显
*@retval:		true:显示成功	false:表示显示失败,可能是非法的字号大小
*********************************************************************/
bool OLED_ShowChar(uint8_t X,uint8_t Y,uint8_t Char,uint8_t Fonty,bool OP)
{
	...
	uint8_t Char_Size=(Fonty/8+((Fonty%8)?1:0))*(Fonty/2);	//得到字体一个字符对应点阵集所占的字节数
	Char=Char-' ';										//获取与' '的偏移量
	for(Byte_ctr=0;Byte_ctr<Char_Size;Byte_ctr++)
	{
		switch(Fonty)
		{
			case 12:	Data=ASCII_1206[Char][Byte_ctr];break;				//调用1206ASCII字体
			case 16:	Data=ASCII_1608[Char][Byte_ctr];break;				//调用1608ASCII字体
			case 24:	Data=ASCII_2412[Char][Byte_ctr];break;				//调用2412ASCII字体
			default: return false;											//不存在此字库
		}
	...
	if(OLED_STATUS.STA.BIT.RAM_BUSY == 0){OLED_Refresh_GRAM();}				//当外部频繁调用OLED_ShowChar时,避免频繁刷新GRAM

	return true;
}

/************************************************************
*@brief:	OLED设置Printf格式，作为打印的前置函数
*@param:	x:				打印到的OLED起点x坐标
*@param:	y:				打印到的OLED起点y坐标
*@param:	sizey:			打印出的字体字号大小
*@param:	mode:			打印的字体的显示模式(正常或是反显)
*@retval:	None
*@note:		该函数应为OLED_Printf的前置函数
************************************************************/
...
/************************************************************
*@brief:	适用于OLED的printf,功能与超级终端等同
************************************************************/
...
*@param:		Float:		欲显示的浮点数
*@retval:		None
*@note:			保留小数点后的位数由OLED_Float_Reserved决定
*********************************************************************/
...
/*********************************************************************
*@brief:		OLED显示单个中文汉字(16字号)
*@param:		X:				字符X坐标,该数值的有效范围0~127
*@param:		Y:				字符对应页坐标,该数值的有效范围0~255
*@param:		*s:				汉字字符
*@param:		OP:				是否反显
*@retval:		true:	打印成功		false:	打印失败，可能是非法的字号大小
*********************************************************************/
void OLED_ShowChinese16x16(uint8_t X,uint8_t Y,uint8_t *s,bool OP)
{
	...
	HZnum=sizeof(FONT_GB_16)/sizeof(FONT_GB_16_TYPE);				//统计汉字数目
		...
		if((FONT_GB_16[Select_Byte].Index[0]==*(s))&&(FONT_GB_16[Select_Byte].Index[1]==*(s+1)))
		{
			for(Byte_ctr=0;Byte_ctr<32;Byte_ctr++)
			{
				...
				for(Bit_ctr=0;Bit_ctr<8;Bit_ctr++)					//逐位写入GRAM
				{
					if((Data<<Bit_ctr)&0x80)
					{
						OLED_EditPixel(X,Y,!OP);					//正常显示
					}
					else
					{
						OLED_EditPixel(X,Y,OP);						//反显
					}
					Y++;
					if((Y-Y0)==16)
					{
...
*@param:		*s:				汉字字符串
*@param:		Sizey:			字体大小
*@param:		OP:				是否反显(false为正常显示，true为开启反显)
*@retval:		true:	打印成功		false:	打印失败，可能是非法的字体大小
*********************************************************************/
...
/*********************************************************************
*@brief:		OLED显示BMP格式图样
*@param:		X0:		图形起始X坐标,该数值的有效范围0~127
*@param:		Y0:		图形起始页坐标,该数值的有效范围0~7
*@param:		X1:		图形末尾X坐标,该数值的有效范围0~127
*@param:		Y1:		图形末尾页坐标,该数值的有效范围0~7
*@param:		BMP[]:	BMP格式图形数组首地址
*@retval:		None
*@note:			正常显示图像需在X0,Y0,X1,Y1都按照原始bmp像素大小写入
*********************************************************************/
...
/*********************************************************************
*@brief:		OLED对比度(亮度)调节函数
*@param:		Level:	调节到的对比度(亮度)值,该数值的有效范围0~255
*@retval:		None
*********************************************************************/
...
/*********************************************************************
*@brief:		OLED设置GRAM在Y方向上的偏移
*@param:		Level:	欲设置GRAM中偏移的值,该数值的有效范围0~63
*@retval:		None
*********************************************************************/
...
/*********************************************************************
*@brief:		OLED全屏反显
*@param:		OP:		是否开启全屏反显,true表示是,false表示否
*@retval:		None
*********************************************************************/
...
void OLED_Init(void)
{
	...
	OLED_WriteReadByte(0xA1,Write_Command);//--Set SEG/Column Mapping     			0xa0左右反置 0xa1正常
	OLED_WriteReadByte(0xC8,Write_Command);//Set COM/Row Scan Direction   			0xc0上下反置 0xc8正常
